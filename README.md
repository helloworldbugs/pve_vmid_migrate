...
- [x] 快照文件迁移（qcow2 内部快照 + state 文件，QEMU/LXC）
...
1. **配置文件** — 重命名 `.conf` 并替换内部 VMID 引用（磁盘路径、vmid 参数、快照 state 文件路径）
2. **磁盘文件** — 解析配置中的存储引用，跨存储后端移动并重命名磁盘文件
3. **快照文件** — 迁移 qcow2 内部快照（随磁盘文件自动迁移）和 RAM 状态文件（`vm-X-state-Y.raw`）
4. **备份文件** — 重命名 `vzdump-*` 备份及其关联文件（日志、notes、protected）
...
- **快照**：qcow2 内部快照随磁盘文件自动迁移；RAM 状态文件（`vm-X-state-*.raw`）会被检测并重命名；配置文件中 `[snapshot]` 段内的路径引用会被更新
