#!/bin/bash
# 在pve8.4.1上测试通过
# 使用方法 bash lxc_id_migrate.sh <原ID> <新ID>

###########################|用户配置区|##################################

# 配置文件目录
CONFIG_DIR="/etc/pve/nodes/PVE/lxc"
# 镜像文件目录
...
# 参数校验
if [ $# -ne 2 ]; then
    echo -e "\033[31m[错误] 用法: $0 <原ID> <新ID>\033[0m"
    exit 1
fi
...
echo -e "${YELLOW}提示：如遇到报错，请检查是否按照要求使用bash执行，本脚本不兼容sh！！！"
...
check_vm_status() {
    ...
    if [ ! -f "${CONFIG_DIR}/${OLD_VMID}.conf" ]; then
        echo -e "${RED}[错误] 原lxc容器${OLD_VMID}不存在${NC}" >&2
        exit 2
    fi

    # 检查新ID是否被占用
    if [ -f "${CONFIG_DIR}/${NEW_VMID}.conf" ]; then
        echo -e "${RED}[错误] 新lxc容器ID${NEW_VMID}已被占用${NC}" >&2
        exit 3
    fi

    # 检查lxc容器状态（需完全关闭）
    local status=$(pct status $OLD_VMID | awk '{print $2}')
    if [ "$status" != "stopped" ]; then
        echo -e "${RED}[错误] lxc容器${OLD_VMID}未关闭，请手动关闭后重试${NC}" >&2
        exit 4
    fi
}
...
migrate_backups() {
    ...
    # 遍历所有匹配的备份文件
    for file in vzdump-lxc-${OLD_VMID}-*; do
        if [ -f "$file" ]; then
...
echo -e "${GREEN}"
echo -e "操作成功！"
echo -e "原ID: ${OLD_VMID} 切换到新ID: ${NEW_VMID}"
echo -e "${NC}"
